-- =============================================================================
-- 若依商城系统 - 完整数据库初始化脚本 (ry_mall_complete.sql)
-- 包含：创建数据库 + 若依基础表 + 商城业务表 + 菜单权限 + 购物车
--
-- 使用方式（命令行）：
--   mysql -u root -p < ry_mall_complete.sql
-- 或在客户端中直接执行本文件
--
-- 默认账号：admin / admin123 ，测试用户：ry / admin123
-- 数据库名需与 application-druid.yml 中 jdbc url 一致（默认 ry-vue）
-- =============================================================================

-- ----------------------------
-- 创建数据库并切换
-- 如需完全重装，可取消下一行注释（会删除已有数据）
