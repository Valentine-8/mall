-- Lite RAG knowledge base (MySQL chunks + OpenAI-compatible embeddings)
-- Run after mall_chat.sql: mysql -u root -p ry-vue < sql/mall_knowledge.sql

-- Extend chat config for RAG (ignore errors if columns already exist)
ALTER TABLE mall_chat_config
    ADD COLUMN kb_enabled CHAR(1) DEFAULT '1' COMMENT 'Knowledge base RAG 0/1' AFTER transfer_keywords;
ALTER TABLE mall_chat_config
    ADD COLUMN embedding_model VARCHAR(128) DEFAULT 'text-embedding-v3' COMMENT 'Embedding model' AFTER kb_enabled;
ALTER TABLE mall_chat_config
    ADD COLUMN rag_top_k INT(11) DEFAULT 3 COMMENT 'Top K chunks per query' AFTER embedding_model;
ALTER TABLE mall_chat_config
    ADD COLUMN chunk_size INT(11) DEFAULT 500 COMMENT 'Chunk char size' AFTER rag_top_k;
ALTER TABLE mall_chat_config
    ADD COLUMN chunk_overlap INT(11) DEFAULT 80 COMMENT 'Chunk overlap chars' AFTER chunk_size;

UPDATE mall_chat_config SET kb_enabled = '1', embedding_model = 'text-embedding-v3',
    rag_top_k = 3, chunk_size = 500, chunk_overlap = 80
WHERE config_id = 1 AND (embedding_model IS NULL OR embedding_model = '');
